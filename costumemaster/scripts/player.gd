...
func _ready() -> void:
	...
	# universe.connect("request_player_lock", self, "_block_movement")
...
func _physics_process(delta) -> void:
	...
	# If we are moving, update our velocity and clamp it to our max speed with respect to delta time and the mass.
	if move_vector != Vector2.ZERO and _can_move:
		...
		_velocity += move_vector * acceleration * MASS * delta
		_velocity = _velocity.clamped(MAX_SPEED * MASS * delta)
